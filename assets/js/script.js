...
if (procceed === null ){ //Checks whether the user pressed "Cancel" to the first prompt, in which case terminates the generation of the password with an alert displayed on the window
...
function validatePWlength (){ //This previously invoked function produces output that guides the user through the procees of validating the length of the password.
...
if (passwordLength >= 8 && passwordLength <= 128){//Checks if the desired length is within the allowed interval.
 ...
 if(genPassword){
  ...
  window.alert("Ok. See you some other time.");//If the user' answer is negative, then the function assigns "undefined" to "passwordLength"
...
var chooseType; //This variable will store the type (lower-case letter, upper-case letter, numeric, or special symbol) that the user chooses for each of the randomly generated characters. The variable will be assigned a value a "passwordLength" amount of times.

var RandomC; //This variable will store a randomly generated character that will be included in the password's string. The variable will be assigned a value a "passwordLength" amount of times.
...
if(passwordLength === undefined){ //Checks if the variable is undefined. In that case, the alert below is displayed and the password generation is stopped.
  ...
  var Length = Number(passwordLength); //Change the string-typed numeral stored in "passwordlength" to a number and store the result in Length.
  
    console.log(Length);//Logs the value stored in "Length" to the console.
  
  var passwordArray = new Array(Length); //Uses the array constructor to create an array of length "Length". Its entries will be each of the randomly generated characters that will form the password string in the order they where generated.
  ...
  //Displays an alert on the window indicating the four possible types of characters the user can choose amongst for each of the randomly generated characters that will be stored in RandomC.
  ...
  for (i = 0; i < passwordArray.length; i++) {//Each iteration of this loop will generate each of the random characters that will be concatenated to form the password string.
  
  var chooseType = window.prompt("Choose the type of character you want for your password string: L, U, N, or S.");//Assigns the input entered by the user after each prompt as the value of the chooseType variable.
  
  console.log(chooseType);//Logs the value stored in the variable "chooseType" after each iteration of the loop. If the user's input is valid, then a random character will be generated.
  ...
  if (chooseType === null){
  ...
  } else if(chooseType === 'L'|| chooseType === 'U'|| chooseType === 'N' || chooseType =='S'){ //Provided the user entered a value, the condition in the "else if" checks wheter the input is valid.
  
    genRandomC(chooseType); //If the input is valid, then the hoisted "genRandomC()" function" is invoked, returning a value to be stored in the "RandomC" variable.

    console.log(RandomC);//The value stored in "RandomC" at the i-th iteration will be logged to the console in "ChromeDevTools".

    passwordArray[i] = RandomC; //The value (randomly generated character) will be stored as the i-th element of "passwordArray"
  ...
  else{//Assuming the input is not validated through the above condition, an alert will be displayed and the user will be given an additional chance to enter valid input.
    ...
    if(chooseType === 'L'|| chooseType === 'U'|| chooseType === 'N' || chooseType === 'S'){//If the input entered during the second try is valid, then the same commands in the previously skipped "else-if" section will be executed 
    ...
    } else{//If the user fails to enter a valid input a second time, then an alert message is displayed and the generation of the random password is stopped.
...
var passwordText = document.querySelector("#password"); //Uses "querySelector()" to get a reference to the element in the DOM with id="password" and stores that reference in the "passwordText" variable. 

var password; //Declares the variable that will store the password string (i.e., the password) to be displayed in the readonly textarea

function writePassword(){//Function declaration: Creates the password string to be shown as output to the user by concatenating the elements of "passwordArray" in the order they appear.

   password = passwordArray[0]; //Base case for the following loop.

  for(i = 1; i < passwordArray.length ; i++)
  {
    password = password + passwordArray[i];//For each iteration of the loop, concatenate a new character to the password string.
  }

  passwordText.textContent = "Your new secure password is: " + password;//After the condition in the loop evaluates as false, output the message after the assignment operator by using the "".textContent" method to insert the text in the area corresponding to the <textarea> element.
...
console.log(password);//Logs the value of "password" to the console. It must be equal to the password displayed in the textarea of the page.
  ...
  function genRandomC(x){ //Given a string as argument, this function checks if one of the four conditions of strict equality hold. If a condition holds, it returns a random character and stores it in RandomC. Otherwise, it returns nothing.
    if(x ==='L'){
